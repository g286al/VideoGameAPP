package com.example.gamesvideosproject.services;
...
public class DataService {
    public String url = "https://api.rawg.io/api/games?key=b2c55d7a03e94df481afbe6905c2649b";
    private final String initialUrl = "https://api.rawg.io/api/games?key=b2c55d7a03e94df481afbe6905c2649b";
    ...
    private ArrayList<Game> parseGames(JsonArray results) {
        ...
        for (JsonElement element : results) {
            ...
            String ratingsCount = gameObj.get("ratings_count").getAsString();


            games.add(new Game(id, name, releaseDate, imgUrl, rating, genre, ratingsCount));
        }
        return games;
    }
        ...
        if (!chosenGame.AlreadyClicked()) {
            new Thread(() -> {
                try {
                    ...
                    new Handler(Looper.getMainLooper()).post(() -> fetchDataListener.onGameDetailsFetched(chosenGame));
